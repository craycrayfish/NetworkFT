...
def calculate_node_positions(
    ...
    # First get all nodes
    nodes = (
        pd.concat([graph["collection_from"], graph["collection_to"]]).unique().tolist()
    )
    ...
    node_positions = {
        i: [node, np.sin(theta * i), np.cos(theta * i)] for i, node in enumerate(nodes)
    }
    return pd.DataFrame.from_dict(
        node_positions, orient="index", columns=["node", "x", "y"]
    )
...
def calculate_edges(
    ...
    graph = _graph.groupby(["collection_from", "collection_to"], as_index=False).agg(
        {"value": "sum"}
    )

    # Created sorted list so that groupby is agnostic of direction
    graph["collection"] = graph.apply(
        lambda x: ",".join(sorted([x["collection_from"], x["collection_to"]])), axis=1
    )
...
def calculate_node_sizes(_edges: pd.DataFrame) -> pd.DataFrame:
    ...
    sizes = edges_exploded.groupby("collection", as_index=False).agg(
